...
\paragraph{Debugmodus} Der \gls{Debugmodus} ist aktiv, wenn das eigentliche Debuggen stattfindet. Hier kann der Nutzer etwa Variablen inspizieren und den Programmablauf verfolgen. 
		...
		\item[/FA90/] Der Benutzer kann \glspl{Relationale Eigenschaft} bezüglich Variablen als \glspl{bedingter Breakpoint} festlegen.
		\item[/FA100/] \Glspl{bedingter Breakpoint} können an vom Benutzer bestimmte Bereiche in den Programmen gebunden werden.
		\item[/FA110/] Der Benutzer kann \glspl{Relationale Eigenschaft} bezüglich Variablen und arithmetischen Termen über den Variablen als \glspl{Watch-Expression} festlegen.
		...
		\item[/FA140/] Das Produkt bietet die Möglichkeit, eine \gls{Konfigurationsdatei} für einen Lauf zu speichern. Diese beinhaltet festgelegte Eingabewerte, \glspl{Breakpoint}, \glspl{Watch-Expression}, Schrittgröße und die zu debuggenden Programme, sowie die Position im Programmablauf, an denen die Ausführung steht.
		...
		\item[/FA210/] Im \gls{Variableninspektor} können Variablen ausgeblendet werden.
...
\begin{itemize}
    ...
    Der Benutzer möchte einen \gls{Breakpoint} hinzufügen. Hierbei hat er die Möglichkeit, einen \gls{Breakpoint} durch Doppelklick direkt in Zeilen im Programmtext zu setzen.\\
    ...
    Der Benutzer möchte eine \gls{Watch-Expression} bezüglich der von ihm hinzugefügten Programme hinzufügen. Dazu wählt er in der Benutzeroberfläche die Schaltfläche \enquote{+} unter \glspl{Watch-Expression} aus. Hier gibt er einen semantisch korrekten %\gls{Watch-Expression} (siehe Kapitel 12) 
Ausdruck ein. Anschließend kann im selben Feld ein Bereich in einem oder mehreren der Programme angegeben werden, in dem das Produkt die Korrektheit der \gls{Watch-Expression} untersucht. Wird einer oder mehrere dieser Bereiche leer gelassen, wird die Korrektheit der \gls{Watch-Expression} während des gesamten Durchlaufs überprüft.
    
    \item [/AF50/] Codes debuggen: \\
			Der Benutzer möchte zwei (oder mehr) Programmteile auf \glspl{Relationale Eigenschaft} (z.B. Nicht-Interferenz oder Äquivalenz) untersuchen. Dazu fügt er die zu untersuchenden Programme in das Produkt ein, setzt \glspl{Breakpoint} an relevanten Stellen und prüft seine Annahmen, wo die Programme sich unterscheiden könnten, mit Hilfe von \glspl{Watch-Expression} oder \glsplgen{bedingter Breakpoint}. Gegebenenfalls verändert er den Programmtext seiner Programme noch einmal. Dann startet er den \gls{Debugger}. Dieser läuft automatisch bis zum jeweils nächsten (bedingten) \gls{Breakpoint} durch. \\Der Benutzer hat dann die Möglichkeit, pro Zeile(n) oder pro \gls{Breakpoint} vorzugehen. Die Ergebnisse der Variablen und der Auswertung der \glspl{Watch-Expression} werden ihm dauerhaft angezeigt und nach jedem Weitergehen aktualisiert. Am Ende des Debug-Vorgangs werden ihm die Ausgabewerte aller Programme angezeigt.
...
%		\item Verletzung der Vorbedingung: Das Programm lässt sich nur einmal öffnen. Folglich erscheint nur das bereits geöffnete Fenster wieder im Vordergrund.
	...
	\item[/T050/] Speichern einer Konfiguration
		...
		\item Reaktion:	Die aktuelle Konfiguration wird in einer \gls{Konfigurationsdatei} abgespeichert.
		\end{itemize}	
...
\begin{itemize}
	...
	\begin{itemize}
		\item Erfolgsvorbedingung: Es sind mehrere Textfenster vorhanden. Die Stelle, an der der \gls{Breakpoint} gesetzt werden sollen, ist in allen Textfenster noch nicht von einem \gls{Breakpoint} belegt und in allen Fenstern einer Zeile validen Programmtextes \frage{(Wie heißen jetzt unsere "Befehle"?)} zugeordnet. 
		\item Aktion: Der linke Rand neben einem der Textfenster wird doppelt angeklickt.	
		\item Reaktion:	In allen vorhandenen Textfenstern wird auf derselben Höhe ein \gls{Breakpoint} gesetzt.
	\end{itemize}		
	
	\item[/T070/] Löschen eines \glspl{Breakpoint} in einem Textfenster
		\begin{itemize}
		\item Erfolgsvorbedingung: Es ist an der entsprechenden Stelle ein \gls{Breakpoint} gesetzt.	
		\item Aktion: Der zu entfernende \gls{Breakpoint} wird einmal einfach angeklickt.
	...
	\item[/T100/] Löschen aller \glspl{Breakpoint} eines Fensters
		\begin{itemize}
		\item Erfolgsvorbedingung: Es sind im entsprechenden Fenster mehrere \glspl{Breakpoint} vorhanden.
		\item Aktion: \frage{Wo ist der Button in der GUI? Im großen Menü? Im Textfenstermenü?}
		\item Reaktion:	Alle \glspl{Breakpoint} des Fensters werden gelöscht.
		\end{itemize}		
	...
	\item[/T110/] Starten des \gls{Debugmodus}
		...
		\item Reaktion:	Das Produkt wechselt in den \gls{Debugmodus} und läuft in allen Programmen bis zum jeweils ersten \gls{Breakpoint} durch bzw. bis die Bedingung des ersten bedingten Breakpoints erfüllt wird.
		\end{itemize}		
	
	\item[/T120/] Abbrechen des \gls{Debugmodus}
		\begin{itemize}
		\item Erfolgsvorbedingung: Das Produkt befindet sich im \gls{Debugmodus}.
		\item Aktion: Der Stop-Button wird angeklickt.
		\item Reaktion: Das Produkt wechselt in den \gls{Editiermodus}.		
		\end{itemize}		
	
	\item[/T130/] Halten des Debuggers an \glspl{Breakpoint}
		\begin{itemize}
		\item Erfolgsvorbedingung: Das Produkt befindet sich im \gls{Debugmodus}. Es existiert ein Breakpoint in einem Textfenster.
		\item Aktion: Es gibt verschiedene Nutzer-Handlungen, durch die ein \gls{Breakpoint} erreicht werden kann: Der Nutzer startet den Debugmodus mit Klick auf den Play-Button. Der Nutzer macht einen Schritt mittels Klick auf den Step-Button. Der Nutzer setzt das bisherige Debuggen fort mittels Klick auf den Continue-Button.
	...
	\item[/T135/] Halten des Debuggers an (bedingten) \glspl{bedingter Breakpoint}
		\begin{itemize}
		\item Erfolgsvorbedingung: Das Produkt befindet sich im \gls{Debugmodus}. Es existiert ein \gls{bedingter Breakpoint} mit syntaktisch korrekter und erfüllbarer Bedingung. 
		\item Aktion: Es gibt verschiedene Nutzer-Handlungene, durch die ein \gls{bedingter Breakpoint} erreicht werden kann. Der Nutzer startet den Debugmodus mit Klick auf den Play-Button. Der Nutzer macht einen Schritt mittels Klick auf den Step-Button. Der Nutzer setzt das bisherige Debuggen fort mittels Klick auf den Continue-Button.
		\item Reaktion:		
		\end{itemize}	
	...
	\item[/T150/] Einen \gls{Schritt} der gewählten \gls{Schrittgroesse} in allen Fenstern machen
		\begin{itemize}
		\item Erfolgsvorbedingung:
		\item Aktion:
		\item Reaktion:		
...
\vspace{0.7cm}
	...
	\subparagraph{Debugger} Der Debugger nutzt die vom \gls{Interpreter} erzeugten Informationen, um \glspl{Watch-Expression} und \glspl{bedingter Breakpoint} auszuwerten, sowie die üblichen Debugmechanismen zu steuern.
	\subparagraph{Interpreter} Der \gls{Interpreter} nutzt den Syntax-Baum, den der Antlr Parser erzeugt, um damit den Programmablauf der zu debuggenden Programme auszuführen. 

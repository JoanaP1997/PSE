...
\section{Produktfuntionen}
		...
		\begin{itemize}
		\item[/FA10/] Das Produkt ist ein \gls{Debugger} für Quelltexte, welche in der im Kapitel Ergänzungen spezifizierten Sprache geschrieben sind.
		...
		\item[/FA140/] Das Produkt bietet die Möglichkeit, eine \gls{Konfigurationsdatei} für einen Lauf zu speichern. Diese beinhaltet festgelegte Eingabewerte, \glspl{Breakpoint}, \glspl{Watch-Expression}, Schrittgröße und die zu debuggenden Quelltexte, sowie die aktuellen Befehle, an denen die Ausführung steht.
		...
		\item[/FA320/] Die, an den Quelltext gebundenen, \glspl{Breakpoint} verschieben sich mit dem Quelltext, wenn dieser im \gls{Editiermodus} geändert wird.
		\end{itemize}		 		
...
\begin{itemize}
    \item [/AF10/] Hinzufügen von Quelltext:\\ 
	Die Nutzerin möchte einen Programmquelltext hinzufügen, auf welchen sie über die Dateiverwaltung
	ihres Betriebssystems zugreifen kann.\\
	Dazu wählt sie die Schaltfläche \enquote{Datei} der Menüleiste. In der Menüansicht wählt sie anschließend die Fläche \enquote{Neuen Quelltext hinzufügen}.\\
	Daraufhin öffnet sich die Dateiverwaltung seines Betriebssystems und die Benutzerin fordert über diese den gewünschten Quelltext an.
	Der Quelltext erscheint in einem Eingabefenster.\\
	Hierbei sind folgende Ausnahmen zu beachten: Besitzt der hinzuzufügende Quelltext mehr als 100 Zeilen, so benachrichtigt das Produkt die Benutzerin über /A10/.\\
	Besitzt der hinzuzufügende Quelltext mehr als 200 Zeilen, so wird dieser nach /A10/ nicht hinzugefügt, und
	das Produkt benachrichtigt den Benutzer mittels einer Fehlermeldung.\\
    
    \item [/AF20/] Ändern von Quelltext:\\
    Die Nutzerin möchte den von ihr hinzugefügten Quelltext ändern. Befindet sie sich im \gls{Debugmodus}, muss sie diesen zuvor abbrechen. \\
    Um den Code zu ändern kann die Nutzerin den Quelltext direkt im Fenster durch hineinklicken bearbeiten. Außerdem kann die Nutzerin auch weitere Quelltexte hinzufügen oder Quelltexte entfernen.
    
    \item [/AF30/] Setzen von Breakpoints:\\
    Die Nutzerin möchte einen \gls{Breakpoint} hinzufügen. Hierbei hat sie die Möglichkeit, einen \gls{Breakpoint} durch Doppelklick direkt in Zeilen im Quelltext zu setzen.\\
    Die zweite Möglichkeit ist das Setzen eines bedingten Breakpoints. Hierzu wählt die Nutzerin die passende \enquote{+} Schaltfläche aus, fügt einen semantisch korrekten bedingten Breakpoint ein und wählt gegebenenfalls einen Bereich aus.
    
    \item [/AF40/] Hinzufügen von Watch-Expressions:\\
    Die Nutzerin möchte eine \gls{Watch-Expression} bezüglich der von ihr hinzugefügten Quelltexte hinzufügen. Dazu wählt sie in der Benutzeroberfläche die Schaltfläche \enquote{+} unter \glspl{Watch-Expression} aus. Hier gibt sie eine semantisch korrekte \gls{Watch-Expression} (siehe Kapitel 12) ein. Anschließend kann im selben Feld ein Bereich in einem oder mehreren der Quelltexte angegeben werden, in dem das Produkt die Korrektheit der \gls{Watch-Expression} untersucht. Wird dieser Bereich leer gelassen, wird die Korrektheit der \gls{Watch-Expression} während des gesamten Durchlaufs überprüft.
    
    \item [/AF50/] Codes debuggen: \\
			Die Nutzerin möchte zwei (oder mehr) Programmteile auf relationale Eigenschaften (z.B. Nicht-Interferenz oder Äquivalenz) untersuchen. Dazu fügt sie die zu untersuchenden Quelltexte in das Programm ein, setzt \glspl{Breakpoint} an relevanten Stellen und prüft ihre Annahmen, wo die Programme sich unterscheiden könnten, mit Hilfe von \glspl{Watch-Expression} oder \glsplgen{bedingter Breakpoint}. Gegebenenfalls verändert sie den Quelltext ihrer Programme noch einmal. Dann startet sie den \gls{Debugger}. Dieser läuft automatisch bis zum jeweils nächsten (bedingten) \gls{Breakpoint} durch. \\Die Nutzerin hat dann die Möglichkeit, pro Zeile(n) oder pro \gls{Breakpoint} vorzugehen. Nach jedem Weitergehen werden ihr die Ergebnisse der Variablen und der Auswertung der \glspl{Watch-Expression} angezeigt. Am Ende des Debug-Vorgangs werden ihr die Ausgabewerte aller Programme angezeigt.
    
    \item [/AF60/] Speichern eines Durchlaufs: \\
    Die Nutzerin möchte einen von ihr durchgeführten Debug-Lauf speichern, um ihn später zu wiederholen, an dieser Stelle fortzusetzen, oder an einen anderen Benutzer weitergeben. Dazu wählt sie zu einem beliebigen Zeitpunkt, nachdem sie mindestens einen zu debuggenden Quelltext hinzugefügt hat, im Dateimenü den Punkt \enquote{\gls{Konfigurationsdatei} speichern} aus. Anschließend kann die Benutzerin den Speicherort der \gls{Konfigurationsdatei} über ein Pop-Up Fenster auswählen.\\
...
\begin{itemize}
	...
	\item[/T140/] Verändern der Schrittlänge
...
\newpage
    \subsection{Beschreibung der Oberfläche}
        ...
        Befindet sich das Produkt im \gls{Debugmodus}, können die Eingabequelltexte nicht mehr
        bearbeitet werden.
     ...
     \begin{itemize}
     \item[Datei] Dieses Menü enthält die Menüeinträge \enquote{Neu}, \enquote{Konfigurationsdatei laden}, \enquote{Konfigrationsdatei speichern} und \enquote{Neuen Quelltext hinzufügen}. Außerdem kann das Produkt über dieses Menü beendet werden.
...
Für die Entwicklung in Java werden die Java-Entwicklungsumgebungen IntelliJ\textsuperscript{\textcopyright} und Eclipse\textsuperscript{\textcopyright} verwendet.
...
Das heißt \gls{Watch-Expression} werten sich zu einem beliebigen Wert aus, während sich \glspl{bedingter Breakpoint} immer zu einem Wahrheitswert auswerten.

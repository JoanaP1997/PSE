...
\paragraph{Debugmodus} Der \gls{Debugmodus} ist aktiv, wenn das eigentliche Debuggen stattfindet. Hier kann der Nutzer etwa Variablen inspizieren und den Programmablauf verfolgen. 
		...
		\item[/FA15/] Es gibt einen \gls{Debugmodus} und einen \gls{Editiermodus}
		...
		\item[/FA80/] Der Benutzer kann \glspl{Breakpoint} an Zeilen in den Programmen setzen.
                \item[/FA90/] Der Benutzer kann \glspl{Relationale Eigenschaft} bezüglich Variablen als \glspl{bedingter Breakpoint} festlegen. %glspl relationale Eigensch.?
		...
		\item[/FA300/] Das Produkt kann, abhängig von der Zeilenanzahl der Programme, dem Benutzer die \gls{Schrittgroesse} vorschlagen.
		...
		\end{itemize}		 		
 		...
 		\item[/A60/] Das Produkt ersetzt keine Versionskontrolle. Der Nutzer ist selbst dafür verantwortlich, seine Änderungen - zum Beispiel durch Kommentare - zu dokumentieren.
 		\end{itemize}
...
%			Zu diesen gehören die Eingabewerte, Schrittgrößen, Variablenauswahl, Programmcode und Position beider Programme, aber nicht die Ausgabewerte. \\ Weiter müssen alle \gls{Breakpoint} gespeichert werden. Dazu zählen sowohl gesetzte als auch bedingte Breakpoints. \Glspl{bedingter Breakpoint} benötigen keine Speicherung der Zeile, 			sondern die Speicherung eines Bereiches, zweier Variablen der Programmtexte und eines Operators. Dies gilt auch für die \glspl{Watch-Expression}. Zudem wird, falls die Datei aus dem \gls{Debugmodus} erzeugt wurde, der letzte Status aller Programme gespeichert, um diese nach späterem Laden weiter ausführen lassen zu können. 
		...
		Da die Sprache alle primitiven Datentypen von Java unterstützt und diese im Produkt durch äquivalente Java Typen, laufend auf einer JVM (Java Virtual Machine), repräsentiert werden, ergeben sich dieselben Genauigkeiten aus Java auch in unserer Sprache. 
		...
		Durch die automatische Generierung von fehlenden Benutzerangaben für Eingabewerte (FA215), wird der Start des Debugmodus aus dem Editiermodus zu jeder Zeit ermöglicht.
		\end{itemize}
		...
		Das Produkt ist durch objektorientierten Entwurf erweiterbar um zusätzliche Sprachen für die Benutzeroberfläche (neben Deutsch und Englisch), sowie um \gls{Interpreter} für weitere Sprachen.
		\item[/NA20/]Wartbarkeit: \\
		Durch den objektorientierten Entwurf des Produkts und ausführliche Dokumentation dessen
		 können auftretende Fehler leicht beseitigt werden. 
...
Bei der Benutzeroberfläche wird auf zwei Eigenschaften besonders Wert gelegt: auf Übersichtlichkeit, damit der Nutzer alle relevanten Daten und Einstellungen leicht findet, und auf Modifizierbarkeit, damit die Benutzeroberfläche leicht angepasst werden kann, falls das Produkt erweitert wird. \\
...
In allen Anwendungsfällen ist der Benutzer der einzige Akteur. Hierbei ist das Geschlecht des Benutzers ohne Bedeutung für
die Benutzung des Produkts. Daher kann der Benutzer sein Geschlecht zu jeder Zeit frei wählen.
...
%		\item Verletzung der Vorbedingung: Das Programm lässt sich nur einmal öffnen. Folglich erscheint nur das bereits geöffnete Fenster wieder im Vordergrund.
	...
	\begin{itemize}
		...
		\item Reaktion: Die ausgewählte Text-Datei wird in einem neuen Text-Fenster eingefügt.
	\end{itemize}	
\newpage
...
\newpage
	\item[/T370/] Ein- und Ausblenden von Variablen
		\begin{itemize}
		\item Erfolgsvorbedingung: Die Programmtexte ist nach Definition der Sprache Wlang (Definition siehe Ergänzungen) valide. Die ein-/auszublendenden
	...
	\item[/T380/] Korrektes Anzeigen des Scrollbalkens
		\begin{itemize}
		\item Erfolgsvorbedingung: Es befinden sich mindestens so viele Variablen im \gls{Variableninspektor}, dass das dafür vorgesehene Feld in der Benutzeroberfläche nicht ausreicht.

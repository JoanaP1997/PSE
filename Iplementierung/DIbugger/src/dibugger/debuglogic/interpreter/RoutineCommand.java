...
public class RoutineCommand extends Command {
	 ...
	 * @param identifiers the identifiers of the routine arguments
	 * @param expectedTypes the expected types of the routine arguments
	 * @param expectedReturnType the expected return type of the routine
	 */
	public RoutineCommand(GenerationController controller, int linenumber, List<String> identifiers,
	...
	public List<TraceState> run() throws DIbuggerLogicException {
		...
		if (this.expectedTypes.size() != this.args.size()
				|| this.expectedTypes.size() != this.identifiers.size()) {
		...
		int i = 0;
		while (!newScope.isRoutineFinished()) {
		...
		if (returnValue.getType() != this.expectedReturnType) {
	...
	/**
	 * Getter for a single child of the children in the command.

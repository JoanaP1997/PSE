...
Wie bereits in Kapitel X und X beschrieben, benutzt der DIbugger im Paket …. Antlr.Parser extern erzeugte Java-Klassen, um die textuelle Nutzer-Eingabe zu einem ablaufbaren Parse-Tree umzuwandeln. Diese Klassen werden von dem frei verfügbaren Programm Antlr (Version 4) erzeugt.
Ein Parse Tree (manchmal auch Syntax Tree genannt) ist dabei ein Baum mit einer eindeutigen Wurzel, der die syntaktische Struktur eines Textes – in diesem Fall der Nutzereingabe – mit Hilfe einer kontextfreien Grammatik darstellt.
Die Grammatik ist schmematisch in Abbildung XX dargestellt. Eine genauere, Antlr-kompatible Spezifikation der Grammatik findet sich im Anhang //LINK.

Auf Basis der Grammatik generiert Antlr mehrere Java-Klassen, die Grundfunktionen eines Lexers und Parsers bereitstellen.
